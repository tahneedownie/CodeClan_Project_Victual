...
LineChartView.prototype.bindEvents = function(){
    this.options.addEventListener('change', (event)=>{
        ...
        console.log(mineralKey);

        databaseRequest.getForDate(today)
        .then((singleDaysData)=>{
                this.nutrientInfoObject[today] = this.calculateTotal(singleDaysData, mineralKey)
            ;
        })
        .then(()=>{
            databaseRequest.getForDate(yesterday)
            .then((singleDaysData)=>{
                this.nutrientInfoObject[yesterday] = this.calculateTotal(singleDaysData, mineralKey);
        })
       .then(()=>{
                databaseRequest.getForDate(twoDaysAgo)
                .then((singleDaysData)=>{
                    this.nutrientInfoObject[twoDaysAgo] = this.calculateTotal(singleDaysData, mineralKey);
            })
            .then(()=>{
                databaseRequest.getForDate(threeDaysAgo)
                .then((singleDaysData)=>{
                    this.nutrientInfoObject[threeDaysAgo] = this.calculateTotal(singleDaysData, mineralKey);
            })
        })})
        .then(()=>{
            databaseRequest.getForDate(fourDaysAgo)
            .then((singleDaysData)=>{
                this.nutrientInfoObject[fourDaysAgo] = this.calculateTotal(singleDaysData, mineralKey);
        })})
        .then(()=>{
            databaseRequest.getForDate(fiveDaysAgo)
            .then((singleDaysData)=>{
                this.nutrientInfoObject[fiveDaysAgo] = this.calculateTotal(singleDaysData, mineralKey);
        })})
        .then(()=>{
            databaseRequest.getForDate(sixDaysAgo)
            .then((singleDaysData)=>{
                this.nutrientInfoObject[sixDaysAgo] = this.calculateTotal(singleDaysData, mineralKey);
        })})
        .then(()=>{
            this.render();
        });
...
LineChartView.prototype.createRows = function(){
    let i = 0;
    for(let element in this.nutrientInfoObject){
        const row = this.body.insertRow(i);
        const th = document.createElement('th');
        th.textContent = element;
        row.appendChild(th);
        const cell = row.insertCell(1);
        cell.innerHTML = this.nutrientInfoObject[element];
        i++;
    }
}
